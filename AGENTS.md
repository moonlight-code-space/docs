...
- API 管理器仅指 CC Switch 和 Codex++ GUI，配置入口只保留这两个。Codex / Claude Code 是配置后使用的工具，不另开手动配置主线。
- 入门流程固定为填写 Base URL 和 API Key → 获取模型 → 对照模型广场选择完整名称 → 保存并启用。编程接入、JSON/TOML、协议原理和手动配置放内部 drafts，不对新手展开。
- Base URL 默认写通用 `https://faroapi.com/`，不把它限定为 Claude Code 专用；只有 Codex（GPT）配置按对应管理器要求使用 `/v1`。CC Switch 是管理器，不能据此一概判断地址。具体 Codex 示例可保留已核对的 `/v1` 填法，不宣称所有版本必须手动加后缀。
- 真实界面标签加粗；命令、路径和模型 ID 使用代码格式。每段围绕一个动作。
- 新手主线只写完成操作所必需的步骤。不要加测试计费提醒、长上下文成本说明、重复概念解释或非必要警告框；仅保留密钥安全、避免重复付款等重要提醒，并尽量用一句话表达。
